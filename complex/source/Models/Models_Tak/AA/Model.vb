...
#Region "inputs"
    ...
    'p from i+1 part
    Public P_Output As New ModelBase.Value("P_Output", "Model_AA: P_Output", Value.ValueType.Input)
    'p from ah part
    Public Pah As New ModelBase.Value("Pah", "Model_AA: Pah", Value.ValueType.Input)

    'q from i-1 part
    Public Q_Input As New ModelBase.Value("Q_Input", "Model_AA: Q_Input", Value.ValueType.Input)
    ...
    'kidney
    Public Qf As New ModelBase.Value("Qf", "Model_Kidney: Qf", Value.ValueType.Input)

#End Region

#Region "outputs"

    Public dQ As New ModelBase.Value("dQ", "Model_AA: dQ", Value.ValueType.Output)
    Public dTs As New ModelBase.Value("dTs", "Model_AA: dTs", Value.ValueType.Output)
    Public dU As New ModelBase.Value("dU", "Model_AA: dU", Value.ValueType.Output)
    Public R As New ModelBase.Value("R", "Model_AA: R", Value.ValueType.Output)
    Public Ts As New ModelBase.Value("Ts", "Model_AA: Ts", Value.ValueType.Output)
    Public U As New ModelBase.Value("U", "Model_AA: U", Value.ValueType.Output)
    Public V As New ModelBase.Value("V", "Model_AA: V", Value.ValueType.Output)
    Public Q As New ModelBase.Value("Q", "Model_AA: Q", Value.ValueType.Output)
    Public P As New ModelBase.Value("P", "Model_AA: P", Value.ValueType.Output)

    Public Rah As New ModelBase.Value("Rah", "Model_AA: Rah", Value.ValueType.Output)
    Public Qah As New ModelBase.Value("Qah", "Model_AA: Qah", Value.ValueType.Output)
...
#Region "parameters"
    ...
    Public k_Rah As New ModelBase.Parameter("k_Rah", "k_Rah")
...
#Region "Calculate"
    ...
    Public Overrides Sub Cycle()
        ...
        Rah.Value(CurrentStep) = Rah.Value(CurrentStep - 1)
        ...
        'AA -> AH
        Qah.Value(CurrentStep) = (P.Value(CurrentStep - 1) - Pah.Value(CurrentStep - 1)) / Rah.Value(CurrentStep - 1)

        'calculating dQ 
        dQ.Value(CurrentStep) = Q_Input.Value(CurrentStep - 1) - Q.Value(CurrentStep) - Qah.Value(CurrentStep) - Qf.Value(CurrentStep - 1) 
        'calculating V --------------------------------------------------------
        ...
        If CurrentTime >= Time_Establishment.Value Then
            ...
            Rah.Value(CurrentStep) = Rah.Value(StepTh) * k_Rah.Value * Sqr(V.Value(StepTh) / V.Value(CurrentStep))
        Else
            R.Value(CurrentStep) = R.Value(CurrentStep - 1)
            Rah.Value(CurrentStep) = Rah.Value(CurrentStep - 1)
        End If
