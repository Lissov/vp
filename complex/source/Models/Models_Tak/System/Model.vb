...
#Region "inputs"
    ...
    Public Air_O2 As New ModelBase.Value("Air_O2", "Air_O2", Value.ValueType.Input)
    Public Air_CO2 As New ModelBase.Value("Air_CO2", "Air_CO2", Value.ValueType.Input)
...
#Region "outputs"
    ...
    Public External_O2 As New ModelBase.Value("External_O2", "System: External_O2", Value.ValueType.Output)
    Public O2 As New ModelBase.Value("O2", "System: O2", Value.ValueType.Output)
    Public O2_Vein As New ModelBase.Value("O2_Vein", "System: O2_Vein", Value.ValueType.Output)
    'Public Fl As New ModelBase.Value("Fl", "System: Fl", Value.ValueType.Output, 15, 30)
    Public Fl As New ModelBase.Value("Fl", "System: Fl", Value.ValueType.Output)
...
#Region "parameters"
    ...
    Public Fl_CO2 As New ModelBase.Parameter("Fl_CO2", "Fl_CO2")
    ...
    Public O2_k3 As New ModelBase.Parameter("O2_k3", "O2_k3")
    Public O2_T As New ModelBase.Parameter("O2_T", "O2_T")
    Public External_O2_k1 As New ModelBase.Parameter("External_O2_k1", "External_O2_k1")
    Public O2_Vein_k1 As New ModelBase.Parameter("O2_Vein_k1", "O2_Vein_k1")
    Public CO2_Th As New ModelBase.Parameter("CO2_Th", "CO2_Th")
    Public CO2_k1 As New ModelBase.Parameter("CO2_k1", "CO2_k1")
...
#Region "Calculate"
    ...
    Public Overrides Sub BeforeCalculate()
        ...
        'Fl.MinValue = 0
        'Fl.InitValue = 7
        Fl.MaxValue = 1.3 * Fl.InitValue
        Fl.MinValue = 0.8 * Fl.InitValue
    ...
    Public Overrides Sub Cycle()
        ...
        'calculate External O2 ------------------------------------------
        External_O2.Value(CurrentStep) = Fl.Value(CurrentStep - 1) * Air_O2.Value(CurrentStep - 1) * External_O2_k1.Value
        'fix Qls to 0.006
        ...
        'calculate O2 --------------------------------------------
        ' O2.Value(CurrentStep) = Hb.Value(CurrentStep - 1) * Fl.Value(CurrentStep - 1) * Q_LS.Value(CurrentStep - 1) * O2_k1.Value
        'O2.Value(CurrentStep) = Hb.Value(CurrentStep - 1) * External_O2.Value(CurrentStep) * O2_k1.Value
        'O2.Value(CurrentStep) = (O2_Vein.Value(CurrentStep - 1) + (External_O2.Value(CurrentStep) - O2.Value(CurrentStep - 1)) * Hb.Value(CurrentStep - 1) * O2_k2.Value) * O2_k1.Value
        Dim dO2 As Double
        dO2 = -O2_k1.Value * O2.Value(CurrentStep - 1) + _
              O2_k2.Value * O2_Vein.Value(CurrentStep - 1) + _
              O2_k3.Value * (External_O2.Value(CurrentStep - 1) - O2.Value(CurrentStep - 1)) * Hb.Value(CurrentStep - 1)
        O2.Value(CurrentStep) = O2.Value(CurrentStep - 1) + [Step] * dO2 * O2_T.Value


        'calculate VsO2 ------------------------------------------
        VsO2.Value(CurrentStep) = (1 + VsO2_k1.Value * Pm.Value(CurrentStep)) / _
                                  (VsO2_k2.Value + VsO2_k2.Value / O2.Value(CurrentStep))
        ...
        'calculate fl --------------------------------------------
        Dim dCo2 As Double
        CO2.Value(CurrentStep) = E.Value(CurrentStep) * CO2_k1.Value
        dCo2 = CO2.Value(CurrentStep - 1) - CO2_Th.Value
        ...
        'calculate O2_Vein  --------------------------------------------
        Dim dO2_Vein As Double
        dO2_Vein = O2_k1.Value * O2.Value(CurrentStep - 1) - _
                   O2_k2.Value * O2_Vein.Value(CurrentStep - 1) - _
                   Vs.Value(CurrentStep - 1) * O2_Vein_k1.Value
        O2_Vein.Value(CurrentStep) = O2_Vein.Value(CurrentStep - 1) + [Step] * dO2_Vein * O2_T.Value
    End Sub
